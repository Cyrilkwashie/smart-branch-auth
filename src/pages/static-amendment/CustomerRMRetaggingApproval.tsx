...
const CustomerRMRetaggingApproval: React.FC = () => {
  const [customerId, setCustomerId] = useState("");
  const [branch, setBranch] = useState("");
  const [customerName, setCustomerName] = useState("");
  const [customerData, setCustomerData] = useState<CustomerRMInfo[]>([]);
  ...
  const loadCustomerData = (id: string) => {
    ...
    setTimeout(() => {
      // Mock customer RM data
      const mockData: CustomerRMInfo[] = [
        {
          customerId: `RM${id}001`,
        ...
        <div className="flex-1 flex flex-col min-w-0 relative overflow-hidden pt-16 sm:pt-20">
          ...
          <main className="p-6">
            <div className="max-w-7xl mx-auto space-y-6">
              ...
              {/* Customer RM Information Table */}
              <Card className="border shadow-sm">
                <CardHeader className="bg-muted/30">
                  <CardTitle className="text-base">Customer RM Information</CardTitle>
                </CardHeader>
                <CardContent className="p-6">
                        ...
                        {isLoading ? (
                          <TableRow>
                            <TableCell colSpan={7} className="text-center text-gray-500 py-8">
                              Loading customer RM data...
                            </TableCell>
                          </TableRow>
                        ) : customerData.length === 0 ? (
                          <TableRow>
                            <TableCell colSpan={7} className="text-center text-gray-500 py-8">
                              {customerId.trim() ? "No customer RM data found for this ID." : "Enter a Customer ID to view customer RM information."}
                            </TableCell>
                          </TableRow>
                        ) : (
...
export default CustomerRMRetaggingApproval;